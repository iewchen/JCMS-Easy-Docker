...
APP_WWW = os.path.join(APPDIR, 'www')
APP_WWW_STATIC = os.path.join(APP_WWW, 'static')
APP_TMP = os.path.join(APPDIR, 'tmp')

fp = open(CFG_FILE, 'r')
cfg = jsonload(fp)

SECRET = '\xa9\xa0\x93\xeb\xc7(\x8e\xc5\x0e\x1c\xd3\xeax\xa8\xa9\xe6v2\xfe\x1af\xa0\x81\x17'
app = Flask(__name__)
...
#cache = MemcachedCache(['127.0.0.1:11211'], key_prefix='rhcs_')
cache = redis.Redis(host=cfg['redis_host'], port=cfg['redis_port'], db=0)
CACHE_LIFE = 5  # 5 sec
...
def encode_passwd(cleartxt):
    ...
    return b64encode(sha256(cleartxt).digest())
...
def get_allmice():
    ...
    racks = {}
    for cid,cage in cages.iteritems():
    ...
    for rackname, rack in racks.iteritems():
        # keep only cage_id in rack
        for r,r_cages in rack.iteritems():
